...
import static com.ekumfi.admin.fragment.AccountFragment1.name;
import static com.ekumfi.admin.fragment.AccountFragment1.profile_image_file;
import static com.ekumfi.admin.fragment.AccountFragment2.latitude;
import static com.ekumfi.admin.fragment.AccountFragment2.longitude;
import static com.ekumfi.admin.fragment.AccountFragment2.primary_contact;
...
import com.ekumfi.admin.pagerAdapter.AccountPageAdapter;
...
public class AccountActivity extends PermisoActivity {
    ...
    public static RealmEkumfiInfo realmEkumfiInfo = new RealmEkumfiInfo();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (getIntent().getStringExtra("MODE").equals("EDIT")) {
            ...
            realmEkumfiInfo = Realm.getInstance(RealmUtility.getDefaultConfig(AccountActivity.this)).where(RealmEkumfiInfo.class).findFirst();
            if (realmEkumfiInfo != null) {
                ekumfi_info_id = realmEkumfiInfo.getEkumfi_info_id();
            }
        }
        ...
        movenext.setOnClickListener(v -> {
            ...
            if (tabFrag2.validate()) {
                if (isNetworkAvailable(AccountActivity.this)) {
                    if (ekumfi_info_id != null && !ekumfi_info_id.equals("")) {
                        new updateEkumfiInfoAsync(getApplicationContext()).execute();
                    } else {
                        new addEkumfiInfoAsync(getApplicationContext()).execute();
                    }
                } else {
    ...
    private class addEkumfiInfoAsync extends AsyncTask<Void, Integer, String> {

        HttpClient httpClient = new DefaultHttpClient();
        private Context context;

        private addEkumfiInfoAsync(Context context) {
        ...
        protected String doInBackground(Void... voids) {
            ...
            String URL = API_URL + "ekumfi-infos";
            try {
                ...
                multipartEntityBuilder.addTextBody("name", name.getText().toString().trim());
...
//                e.printStackTrace();
                ...
                if (result.contains("connect")){
                    ...
                    try {
                        ...
                        Realm.getInstance(RealmUtility.getDefaultConfig(AccountActivity.this)).executeTransaction(realm -> {
                            try {
                                RealmEkumfiInfo realmEkumfiInfo = realm.createOrUpdateObjectFromJson(RealmEkumfiInfo.class, jsonObject.getJSONObject("ekumfi_info"));
    ...
    private class updateEkumfiInfoAsync extends AsyncTask<Void, Integer, String> {

        HttpClient httpClient = new DefaultHttpClient();
        private Context context;

        private updateEkumfiInfoAsync(Context context) {
            this.context = context;
        }
        ...
        protected String doInBackground(Void... voids) {
            ...
            HttpEntity httpEntity = null;
            String responseString = null;
            String URL = API_URL + "ekumfi-infos/" + ekumfi_info_id;
            try {
                HttpPost httpPost = new HttpPost(URL);
                MultipartEntityBuilder multipartEntityBuilder = MultipartEntityBuilder.create();
            ...
            if (result != null) {
                if (result.contains("connect")){
                    ...
                    try {
                        JSONObject jsonObject = new JSONObject(result);
                        Realm.getInstance(RealmUtility.getDefaultConfig(AccountActivity.this)).executeTransaction(realm -> {
                            realm.createOrUpdateObjectFromJson(RealmEkumfiInfo.class, jsonObject);
