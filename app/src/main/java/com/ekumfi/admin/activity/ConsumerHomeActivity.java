...
import com.ekumfi.admin.fragment.CartListFragment;
import com.ekumfi.admin.fragment.ChatIndexFragment;
import com.ekumfi.admin.fragment.ConsumerSettingsFragment;
import com.ekumfi.admin.fragment.ProductsFragment;
import com.ekumfi.admin.fragment.SearchProductFragment;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Realm.getInstance(RealmUtility.getDefaultConfig(ConsumerHomeActivity.this)).executeTransaction(realm -> {
            RealmResults<RealmCart> realmCarts = realm.where(RealmCart.class)
                    .distinct("seller_id")
                    .equalTo("consumer_id", PreferenceManager.getDefaultSharedPreferences(getApplicationContext()).getString("CONSUMERID", ""))
                    .findAll();
            for (RealmCart realmCart : realmCarts) {
                FirebaseMessaging.getInstance().subscribeToTopic(realmCart.getSeller_id() + PreferenceManager.getDefaultSharedPreferences(getApplicationContext()).getString("CONSUMERID", ""))
                        .addOnCompleteListener(task -> {
                            String msg = "successfully subscribed";
                            if (!task.isSuccessful()) {
...
//        realm.createOrUpdateAllFromJson(RealmDrawingCoordinate.class, responseJson.getJSONArray("drawing_coordinates"));
    ...
    public static void retriev_current_registration_token(Context context, String confirmation_token) {
        ...
        JsonObjectRequest jsonObjectRequest = new JsonObjectRequest(
                ...
                keyConst.API_URL + "update-confirmation-token/" + PreferenceManager.getDefaultSharedPreferences(context).getString(MYUSERID, ""),
                request,
                response -> {
                },
                error -> {
