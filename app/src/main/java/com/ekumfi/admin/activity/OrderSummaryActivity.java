...
import static com.ekumfi.admin.activity.ConsumerAccountActivity.*;
import static com.ekumfi.admin.activity.AccountActivity.*;
...
import com.ekumfi.admin.materialDialog.ChoosePaymentMethodMaterialDialog;
...
public class OrderSummaryActivity extends PermisoActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        role = PreferenceManager.getDefaultSharedPreferences(getApplicationContext()).getString("ROLE", "");
        ...
        edit.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (role.equals("CONSUMER")) {
                    realmConsumer = Realm.getInstance(RealmUtility.getDefaultConfig(orderSummaryActivity)).where(RealmConsumer.class).equalTo("consumer_id", PreferenceManager.getDefaultSharedPreferences(orderSummaryActivity).getString("CONSUMERID", "")).findFirst();
                    startActivity(new Intent(getApplicationContext(), ConsumerAccountActivity.class)
                            .putExtra("MODE", "EDIT")
                    );
                } else {
//                    realmEkumfiInfo = Realm.getInstance(RealmUtility.getDefaultConfig(orderSummaryActivity)).where(RealmSeller.class).equalTo("seller_id", PreferenceManager.getDefaultSharedPreferences(orderSummaryActivity).getString("SELLER_ID", "")).findFirst();
    ...
    protected void onResume() {
        ...
        if (role.equals("CONSUMER")) {
            ...
            Realm.init(getApplicationContext());
            realmConsumer[0] = Realm.getInstance(RealmUtility.getDefaultConfig(orderSummaryActivity)).where(RealmConsumer.class).equalTo("consumer_id", PreferenceManager.getDefaultSharedPreferences(orderSummaryActivity).getString("CONSUMERID", "")).findFirst();
            ...
            Realm.init(getApplicationContext());
            realmSeller[0] = Realm.getInstance(RealmUtility.getDefaultConfig(orderSummaryActivity)).where(RealmSeller.class).equalTo("seller_id", PreferenceManager.getDefaultSharedPreferences(orderSummaryActivity).getString("SELLER_ID", "")).findFirst();
